...
if (x == "/login") {
  if ($.urlParam("username"))
    document.querySelector("#username").value = $.urlParam("username");
  ...
  if (!isclickedSignup) {
    ...
    $.post(url, data, (res) => {
      if (res.success === 1) {
        ...
        setTimeout(() => {
          window.location.href = `/login?username=${input.val()}`;
        }, 1500);
      } else if (res.error === 1) {
        $(e).addClass("disabled");
        ...
        if (res.alternatives) {
          ...
          alts.forEach((alt) => {
            if (alt !== null) {
              ...
              a =
                '<span class="alt" style="margin-right:8px">' + alt + "</span>";
              adiv.append(a);
            }
          });
          $(".alt").on("click", (ee) => {
          ...
          // have no alternatives but there is error
          message = res.message;
          popup(message, "auto", "warning", 3000);
        }
      } else {
        message =
          'There is an critical error. Please tell me this error <a href="mailto:abdikaangrbz@gmail.com">at here</a>';
